...
def ode87(f,t_ini,t_end,Y0,criterion,tol=1e-12,propagate=False,minimal_nb_of_points=10):
    """
    ...
    while 0 < (t_end-c_time)*sign and criterion(Y[-1,:]):
...
def ode_rk4(f,Y0,t,criterion):
    """
    Renvoie l'intégrations de l'équation différentielle y' = f(y,t) calculée via une méthode de 
    Runge-Kutta d'ordre 4, quelque soit la nature de y (scalaire, matrice, vecteur, etc)
    
    Parameters
    ----------
    
    f: function
        La fonction définissant la dérivée de y
    Y0 : ndarray
        La valeur de y à l'instant initial t0
    t : 1-D array
        L'intervalle sur lequel on intègre y', de la forme [t0,tf] 
        
    Returns
    -------
    
    Y: ndarray
        La fonction y intégrée sur l'intervalle t avec la dépendance 
        temporelle sur le premier axe du ndarray
        
    Examples
    --------
    
    >>> t = np.linspace(0,1,1000)
    >>> ode_rk4(lambda y,t: -y,1,t)
    array([ 1.        ,  0.98994983,  0.98000067, ...,  0.37538693,
        0.37161423,  0.36787944])
    """
    n = len(t)
    Y = np.zeros((len(t),)+np.shape(Y0),dtype=Y0.dtype)
    Y[0] = Y0
    ...
    for i in range(n-1):
        ...
        k2 = f(t[i]+h,Y[i]+h*k1)
        k3 = f(t[i]+h,Y[i]+h*k2)
        k4 = f(t[i]+2*h,Y[i]+2*h*k3)
        
        Y[i+1] = Y[i] + h/3*(k1+2*k2+2*k3+k4)
